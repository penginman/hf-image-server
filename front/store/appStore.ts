...
import { 
    ...
    StorageType, S3Config, WebDAVConfig, ServiceMode, VideoSettings, CustomProvider, GenerationChannel
} from '../types';
...
interface AppState {
    ...
    manualChannel: GenerationChannel;
    ...
    setGenerationResolution: (resolution: { width: number, height: number }) => void;
    setGenerationChannel: (channel: GenerationChannel) => void;
    setRouteStrategy: (strategy: 'auto' | 'manual') => void;
    setManualChannel: (channel: GenerationChannel) => void;
    setHfAccessToken: (token: string) => void;
...
export const useAppStore = create<AppState>()(
    persist(
        (set, get) => ({
            ...
            generationResolution: getLocalItem<{ width: number, height: number }>('app_generation_resolution', { width: 1536, height: 1536 }),
            generationChannel: getLocalItem<GenerationChannel>('app_generation_channel', 'public'),
            routeStrategy: getLocalItem<'auto' | 'manual'>('app_route_strategy', 'auto'),
            manualChannel: getLocalItem<GenerationChannel>('app_manual_channel', 'public'),
            hfAccessToken: getLocalItem<string>('app_hf_access_token', ''),
            ...
            setGenerationResolution: (generationResolution) => set({ generationResolution }),
            setGenerationChannel: (generationChannel) => set({ generationChannel }),
            setRouteStrategy: (routeStrategy) => set({ routeStrategy }),
            setManualChannel: (manualChannel) => set({ manualChannel }),
            setHfAccessToken: (hfAccessToken) => set({ hfAccessToken }),
            ...
            resetSettings: () => set({
                ...
                seed: '',
                aspectRatio: '1:1',
                generationResolution: { width: 1536, height: 1536 },
                generationChannel: 'public',
                routeStrategy: 'auto',
                manualChannel: 'public',
                hfAccessToken: '',
                currentImage: null,
                isLiveMode: false,
                error: null,
        ...
        }),
        {
            ...
            partialize: (state) => ({
                ...
                generationResolution: state.generationResolution,
                generationChannel: state.generationChannel,
                routeStrategy: state.routeStrategy,
                manualChannel: state.manualChannel,
                hfAccessToken: state.hfAccessToken,
                seed: state.seed,
                steps: state.steps,
