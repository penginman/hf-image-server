...
import { getClosestAspectRatio, normalizeResolution } from '../utils/resolution';
...
export const CreationView: React.FC = () => {
    const { 
        ...
        generationChannel, hfAccessToken,
        routeStrategy, manualChannel,
        prompt, setPrompt,
        generationResolution, seed, steps, setSteps, guidanceScale, setGuidanceScale,
    ...
    const hasRealtimeProgressRef = useRef<boolean>(false);
    ...
    const startProgressTicker = () => {
        ...
        progressTickerRef.current = setInterval(() => {
            setGenerationProgress(prev => {
                if (!prev || hasRealtimeProgressRef.current || prev.percent >= 92) return prev;
                return { ...prev, percent: prev.percent + 1 };
            });
        }, 800);
    };

    const updateGenerationProgress = (event: GradioProgressEvent) => {
        if (event.type === 'close_stream') return;
        if (event.type === 'progress' || event.type === 'heartbeat' || event.type === 'process_completed') {
            hasRealtimeProgressRef.current = true;
        }
        const fallbackStage =
        ...
        setGenerationProgress(prev => {
            const basePercent = prev?.percent ?? 0;
            let nextPercent = basePercent;

            if (typeof event.percent === 'number') {
                nextPercent = Math.max(basePercent, Math.max(0, Math.min(100, Math.round(event.percent))));
            } else if (event.type === 'heartbeat') {
                nextPercent = Math.max(basePercent, 10);
            ...
            return {
                percent: nextPercent,
                desc: event.desc || prev?.desc || null,
                stage: fallbackStage
            };
        });
    };
    ...
    const handleGenerate = async () => {
        const isCustomProvider = !['huggingface', 'gitee', 'modelscope', 'a4f'].includes(provider);
        const requiresHfToken = isCustomProvider && generationChannel === 'hf_token';
        const trimmedHfToken = hfAccessToken.trim();

        if (!prompt.trim()) return;
        if (requiresHfToken && !trimmedHfToken) {
            setError('Please enter Access Tokens: hf_xxxxxxxx...');
            return;
        }
        if (requiresHfToken && !trimmedHfToken.startsWith('hf_')) {
            setError('Access Tokens format looks invalid. It should start with "hf_".');
            return;
        }
        ...
        setIsLiveMode(false);
        hasRealtimeProgressRef.current = false;
        setGenerationProgress({ percent: 3, desc: null, stage: 'Submitting request...' });
        startProgressTicker();
        ...
        try {
            ...
            const normalizedResolution = normalizeResolution(generationResolution.width, generationResolution.height);
            const requestAspectRatio = getClosestAspectRatio(normalizedResolution.width, normalizedResolution.height);
            const supportsResolutionOverride = !(provider === 'huggingface' && model === 'qwen-image');
            // HF qwen-image endpoint accepts aspect ratio but not explicit width/height.
            const requestResolution = supportsResolutionOverride ? normalizedResolution : undefined;

            let result;
            if (provider === 'gitee') {
                setGenerationProgress(prev => prev ? { ...prev, percent: Math.max(prev.percent, 12), stage: 'Generating...' } : prev);
                result = await generateGiteeImage(model, finalPrompt, requestAspectRatio, seedNumber, steps, requestHD, currentGuidanceScale, requestResolution);
            } else if (provider === 'modelscope') {
                setGenerationProgress(prev => prev ? { ...prev, percent: Math.max(prev.percent, 12), stage: 'Generating...' } : prev);
                result = await generateMSImage(model, finalPrompt, requestAspectRatio, seedNumber, steps, requestHD, currentGuidanceScale, requestResolution);
            } else if (provider === 'huggingface') {
                setGenerationProgress(prev => prev ? { ...prev, percent: Math.max(prev.percent, 12), stage: 'Connecting to ZeroGPU...' } : prev);
                const zImageTurboRouting =
                    model === 'z-image-turbo'
                        ? { routeStrategy, manualChannel }
                        : undefined;
                ...
                setGenerationProgress(prev => prev ? { ...prev, percent: Math.max(prev.percent, 12), stage: 'Generating...' } : prev);
                result = await generateA4FImage(model, finalPrompt, requestAspectRatio, seedNumber, steps, requestHD, currentGuidanceScale, requestResolution);
            } else {
                const customProviders = getCustomProviders();
                const activeProvider = customProviders.find(p => p.id === provider);
                if (activeProvider) {
                    setGenerationProgress(prev => prev ? { ...prev, percent: Math.max(prev.percent, 12), stage: 'Generating...' } : prev);
                    const requestHfToken = requiresHfToken ? trimmedHfToken : undefined;
            ...
            setGenerationProgress(prev => prev ? { ...prev, percent: Math.max(prev.percent, 96), stage: 'Processing output...' } : prev);
    ...
    const isCustomProvider = !['huggingface', 'gitee', 'modelscope', 'a4f'].includes(provider);
    const tokenMissingForChannel =
        isCustomProvider &&
        generationChannel === 'hf_token' &&
        !hfAccessToken.trim();
    const tokenFormatInvalidForChannel =
        isCustomProvider &&
        generationChannel === 'hf_token' &&
        !!hfAccessToken.trim() &&
        !hfAccessToken.trim().startsWith('hf_');
                ...
                <div className="flex-grow space-y-4 md:space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                        <button 
                            ...
                            disabled={isWorking || !prompt.trim() || isTranslating || tokenMissingForChannel || tokenFormatInvalidForChannel}
                    ...
                    {isWorking && generationProgress && (
                        <div className="rounded-xl border border-white/10 bg-black/25 px-3 py-2 backdrop-blur">
                            <div className="mb-1 flex items-center justify-between gap-2 text-xs text-white/80">
                                <span className="truncate">{generationProgress.desc || generationProgress.stage}</span>
