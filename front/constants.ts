...
export const getModelConfig = (provider: ProviderOption, model: ModelOption) => {
  if (provider === 'gitee') {
    if (model === 'z-image-turbo') return { min: 4, max: 20, default: 9 };
    if (model === 'qwen-image') return { min: 4, max: 20, default: 20 };
    if (model === 'flux-1-schnell') return { min: 4, max: 20, default: 8 };
    if (model === 'flux-1-krea') return { min: 4, max: 20, default: 20 };
    if (model === 'flux-1') return { min: 4, max: 20, default: 20 };
    if (model === 'flux-2') return { min: 4, max: 20, default: 20 };
  } else if (provider === 'modelscope') {
    if (model === 'z-image-turbo') return { min: 4, max: 20, default: 9 };
    if (model === 'z-image') return { min: 4, max: 20, default: 20 };
    if (model === 'flux-2') return { min: 4, max: 20, default: 20 };
    if (model === 'flux-1-krea') return { min: 4, max: 20, default: 20 };
    if (model === 'flux-1') return { min: 4, max: 20, default: 20 };
  } else if (provider === 'a4f') {
    return { min: 4, max: 20, default: 9 }; // A4F mostly ignores steps via API, using reasonable default
  } else {
    // Hugging Face
    if (model === 'z-image-turbo') return { min: 4, max: 20, default: 9 };
    if (model === 'z-image') return { min: 4, max: 20, default: 20 };
    if (model === 'flux-1-schnell') return { min: 4, max: 20, default: 8 };
    if (model === 'qwen-image') return { min: 4, max: 20, default: 8 };
    if (model === 'ovis-image') return { min: 4, max: 20, default: 20 };
  }
  return { min: 4, max: 20, default: 9 }; // fallback
};
