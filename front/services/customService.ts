    ...
    try {
        ...
        if (!response.ok) {
            if (response.status === 401) {
                const authError = new Error('401') as Error & { status?: number; code?: string; retryable?: boolean };
                authError.status = 401;
                authError.code = 'SERVER_MODELS_AUTH_REQUIRED';
                authError.retryable = false;
                throw authError;
            }

            const details = await readErrorMessage(response);
            const httpError = new Error(details || `Failed to fetch server models (HTTP ${response.status})`) as Error & { status?: number; code?: string; retryable?: boolean };
            httpError.status = response.status;
            httpError.code = 'SERVER_MODELS_FETCH_FAILED';
        ...
        if (error?.code === 'SERVER_MODELS_AUTH_REQUIRED' || error?.code === 'SERVER_MODELS_FETCH_FAILED') {
            throw error;
        }

        const networkError = new Error('Failed to fetch server models: network error') as Error & { code?: string; retryable?: boolean };
        networkError.code = 'SERVER_MODELS_NETWORK_ERROR';
...
export const generateCustomImage = async (
    ...
    const trimmedToken = (hfAccessToken || '').trim();
    if (trimmedToken) {
