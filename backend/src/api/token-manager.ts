...
export function getTokens(provider: Provider, env: Bindings): string[] {
  ...
  switch (provider) {
    case "huggingface":
      tokenSource = env?.HUGGINGFACE_TOKENS;
      break;
    case "gitee":
      tokenSource = env?.GITEE_TOKENS;
      break;
    case "modelscope":
      tokenSource = env?.MODELSCOPE_TOKENS;
      break;
    case "a4f":
      tokenSource = env?.A4F_TOKENS;
      break;
    case "modelslab":
      tokenSource = env?.MODELSLAB_API_KEY;
      break;
  }

  if (typeof tokenSource !== "string") return [];
