...
# Bash API documentation for yingzhac/Z_image_NSFW
API Endpoints: 2
...
```bash
curl -X POST https://yingzhac-z-image-nsfw.hf.space/gradio_api/call/generate_image -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://yingzhac-z-image-nsfw.hf.space/gradio_api/call/generate_image/$EVENT_ID
```
...
```bash
curl -X POST https://yingzhac-z-image-nsfw.hf.space/gradio_api/call/generate_image_1 -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://yingzhac-z-image-nsfw.hf.space/gradio_api/call/generate_image_1/$EVENT_ID
```
