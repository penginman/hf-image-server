...
# Bash API documentation for linoyts/Z-Image-Portrait
API Endpoints: 2
...
```bash
curl -X POST https://linoyts-z-image-portrait.hf.space/gradio_api/call/generate_image -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://linoyts-z-image-portrait.hf.space/gradio_api/call/generate_image/$EVENT_ID
```
...
[0]: - Type: 
- The output value that appears in the "Output (Left-with the LoRA, Right-without)" Imageslider component.
...
```bash
curl -X POST https://linoyts-z-image-portrait.hf.space/gradio_api/call/generate_image_1 -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://linoyts-z-image-portrait.hf.space/gradio_api/call/generate_image_1/$EVENT_ID
```
