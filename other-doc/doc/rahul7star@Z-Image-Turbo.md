...
# Bash API documentation for rahul7star/Z-Image-Turbo
API Endpoints: 1
...
```bash
curl -X POST https://rahul7star-z-image-turbo.hf.space/gradio_api/call/generate_image -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://rahul7star-z-image-turbo.hf.space/gradio_api/call/generate_image/$EVENT_ID
```
...
- Type: number
- Required
- The input value that is provided in the Number of Images Slider component. 
...
[2]: - Type: string
- The output value that appears in the "Debug Log Output" Textbox component.
