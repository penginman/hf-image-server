...
# Bash API documentation for Azure99/Z-Image-Turbo
API Endpoints: 1
...
```bash
curl -X POST https://azure99-z-image-turbo.hf.space/gradio_api/call/generate -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://azure99-z-image-turbo.hf.space/gradio_api/call/generate/$EVENT_ID
```
...
- Type: boolean
- Required
- The input value that is provided in the 返回图片 Checkbox component. 
...
[3]: - Type: string
- The output value that appears in the "Saved Image Path" Textbox component.
