...
# Bash API documentation for AiSudo/Z-Image-to-LoRA
API Endpoints: 2
...
```bash
curl -X POST https://aisudo-z-image-to-lora.hf.space/gradio_api/call/generate_lora -s -H "Content-Type: application/json" -d '{
	"data": [
							[]
						
	]}' \
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://aisudo-z-image-to-lora.hf.space/gradio_api/call/generate_lora/$EVENT_ID
```
...
```bash
curl -X POST https://aisudo-z-image-to-lora.hf.space/gradio_api/call/generate_image -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://aisudo-z-image-to-lora.hf.space/gradio_api/call/generate_image/$EVENT_ID
```
