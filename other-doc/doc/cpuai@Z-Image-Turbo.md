...
# Bash API documentation for cpuai/Z-Image-Turbo
API Endpoints: 2
...
```bash
curl -X POST https://cpuai-z-image-turbo.hf.space/gradio_api/call/update_res_choices -s -H "Content-Type: application/json" -d '{
	"data": [
							"1024"
						
	]}' \
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://cpuai-z-image-turbo.hf.space/gradio_api/call/update_res_choices/$EVENT_ID
```
...
```bash
curl -X POST https://cpuai-z-image-turbo.hf.space/gradio_api/call/generate -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://cpuai-z-image-turbo.hf.space/gradio_api/call/generate/$EVENT_ID
```
