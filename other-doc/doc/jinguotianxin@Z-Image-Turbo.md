...
# Bash API documentation for jinguotianxin/Z-Image-Turbo
API Endpoints: 1
...
### API Name: /generate
Description: Generate an image using the Z-Image model based on the provided prompt and settings.

```bash
curl -X POST https://jinguotianxin-z-image-turbo.hf.space/gradio_api/call/generate -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://jinguotianxin-z-image-turbo.hf.space/gradio_api/call/generate/$EVENT_ID
```
