...
# Bash API documentation for mrfakename/Z-Image-Turbo
API Endpoints: 3
...
```bash
curl -X POST https://mrfakename-z-image-turbo.hf.space/gradio_api/call/toggle_seed -s -H "Content-Type: application/json" -d '{
	"data": [
							true
						
	]}' \
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://mrfakename-z-image-turbo.hf.space/gradio_api/call/toggle_seed/$EVENT_ID
```
...
```bash
curl -X POST https://mrfakename-z-image-turbo.hf.space/gradio_api/call/generate_image -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://mrfakename-z-image-turbo.hf.space/gradio_api/call/generate_image/$EVENT_ID
```
...
```bash
curl -X POST https://mrfakename-z-image-turbo.hf.space/gradio_api/call/generate_image_1 -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://mrfakename-z-image-turbo.hf.space/gradio_api/call/generate_image_1/$EVENT_ID
```
