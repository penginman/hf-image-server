...
# Bash API documentation for mcp-tools/Z-Image-Turbo
API Endpoints: 1
...
```bash
curl -X POST https://mcp-tools-z-image-turbo.hf.space/gradio_api/call/generate -s -H "Content-Type: application/json" -d '{
	"data": [
							"Hello!!",
	...
	| awk -F'"' '{ print $4}'  \
	| read EVENT_ID; curl -N https://mcp-tools-z-image-turbo.hf.space/gradio_api/call/generate/$EVENT_ID
```
